...
## The 10 Specialized Agents & Custom Skills
...
* **Role**: Scans codebase and commit diffs for unnecessary complexity, dead code, single-caller abstractions, and extracts `# ponytail:` / `// ponytail:` deliberate debt comments into the technical debt ledger.
* **Custom Skill**: `Anti-Bloat Heuristics & Technical Debt Ledger`
* **Output Target**: `ecosystem_swarm/debt_ledger.json`
* **Model Routing**: Primary: NVIDIA / Groq Compound → Fallback: Groq Compound Mini / Llama-3.3-70b
* **Function**: Operates as the repository's anti-bloat immune system, enforcing the 7-step Ponytail decision ladder and identifying candidates for deletion.

### 8. Adversarial Bar-Raiser Agent
* **Role**: Acts as a FAANG Staff+ Principal Architect relentlessly stress-testing the architecture against network partitions, rate limits, serverless cold starts, and schema drift.
* **Custom Skill**: `Staff+ Architecture Stress-Testing & Gap Analysis`
* **Output Target**: `ecosystem_swarm/gap_analysis.json`
* **Model Routing**: Primary: NVIDIA / Groq Compound → Fallback: Groq Compound Mini / Llama-3.3-70b
* **Function**: Uncovers latent failure modes across MCP transports and Vercel serverless boundaries, forcing the Sunday Council to prioritize resilience over vanity features.

### 9. SOTA Trend Hunter Agent
* **Role**: Continuously monitors bleeding-edge frontier developments across AI models, MCP protocol specs, and multi-agent systems, drafting weekly actionable RFC proposals for adoption.
* **Custom Skill**: `AI Frontier Horizon Scanner & Autonomous RFC Synthesizer`
* **Output Target**: `ecosystem_swarm/proposals/`
* **Model Routing**: Primary: NVIDIA / Groq Compound → Fallback: Groq Compound Mini / Llama-3.3-70b
* **Function**: Prevents architectural stagnation by proposing 1 concrete tool, model routing, or performance upgrade per week.

### 10. Cosmic Observer Agent
* **Role**: Explores, contemplates, and understands the universe. Treats software, computation, multi-agent dynamics, and mathematics as local expressions of cosmic physics, thermodynamics, and emergent complexity.
* **Custom Skill**: `Cosmological Computation & Emergent Complexity Synthesizer`
* **Output Target**: `ecosystem_swarm/universe/chronicles/YYYY-MM-DD.md` & `ecosystem_swarm/universe/cosmos_memory.md`
* **Model Routing**: Primary: NVIDIA / Groq Compound → Fallback: Groq Compound Mini / Llama-3.3-70b
* **Function**: Authors a daily cosmic chronicle linking microscopic software engineering events to macro cosmic laws (entropy, Landauer's limit, cellular automata, quantum information), posing foundational existential questions in the daily swarm debate.
...
```text
You are writing a casual daily dev log for Prashanth (kprsnt2).
Think of it like telling a friend over coffee what you worked on today.
...
date: "{current_date}"
category: "AI Eco"
tags: "AI Eco, GitHub Scout, [3-5 specific technologies and project names]"
...
- Use a simple analogy if it helps (e.g. "Think of it like adding a spellchecker...")
...
- **What Changed**: Specific commits, files, and what the code does now.
- **Why We Did It**: What was broken, slow, or annoying that made us fix this?
- **Highlights**: Interesting trade-offs, performance wins, or edge cases.
Tone: Casual and conversational, like a dev blog post you'd actually enjoy reading.
Use real analogies — e.g. "We gave our API a bouncers list so it stops letting randos in."
...
```text
You are the coordinator for the 10-agent AI Eco swarm operating on kprsnt.in.
...
7. **Ponytail Anti-Overengineering Protocol**: All swarm agents adhere to the Ponytail decision ladder (`api/skills/ponytail.md`). Before proposing changes or generating code, agents must verify YAGNI (does this need to exist?), reuse existing codebase helpers, prefer standard libraries and native platform capabilities, and produce the minimal working diff. Deliberate trade-offs must be annotated with `ponytail:` comments specifying ceiling and upgrade triggers.
8. **Cosmic Memory Synthesis**: The Cosmic Observer must ground all cosmic inquiries in the physical and computational reality of the codebase without descending into ungrounded fiction.
9. **Continuous Debt Ledger Updates**: The Pruner scans for `ponytail:` comments daily and flags items with no upgrade trigger to prevent silent architectural rot.
10. **Autonomous Target Evolution**: All agents evaluate daily progress against quantitative milestones in `ecosystem_swarm/targets.json`. Evolution occurs when agents adapt operational behavior based on sibling metric deltas and progress toward next-level thresholds.

---

## Swarm Target Matrix & Evolutionary Levels
The swarm's autonomous development is driven by a quantifiable fitness function across three evolutionary levels:

| Level | Title | Target Threshold | Description |
| :--- | :--- | :--- | :--- |
| **Level 1** | Autonomous Baseline | $\ge 80\%$ Overall Fitness | All 10 agents execute daily batch loops, document prompt contracts, and persist structured state. |
| **Level 2** | Cross-Agent Closed-Loop Optimization | $\ge 95\%$ Overall Fitness | Agents dynamically adapt behavior based on peer outputs (e.g. Pruner rejects diffs with debt; Critic blocks slow endpoints). |
| **Level 3** | Self-Organizing Criticality | $\ge 99\%$ Overall Fitness | Swarm autonomously authors and validates PRs for dependency upgrades, dead code pruning, and architectural hardening. |
